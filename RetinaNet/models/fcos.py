...
class FCOS(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # # 2024.09.01 @hslee
        self.gaussian_teacher = transforms.GaussianBlur(kernel_size=3, sigma=(0.1, 2.0))
    ...
    def forward(
        ...
        # 2024.09.01 @hslee
        # 1. original image for S
        original_images = images.tensors.clone()
        
        # 2. gaussian image
        gaussian_images = self.gaussian_teacher(images.tensors.clone())
        
        # # 3. original image + canny edge image
        # images_np = images.tensors.clone().cpu().numpy() # [2, 3, 800, 1216]
        # canny_images = images.tensors.clone()
        # for i, canny_image in enumerate(canny_images):
        #     img = images_np[i]
        #     rgb_img = img.copy().transpose(1, 2, 0)  # Convert to HWC format
        #     # rgb_img to uint8, but keep HWC format
        #     rgb_img = (rgb_img * 255).astype(np.uint8)
        #     canny_edges = cv2.Canny(rgb_img, 100, 200)
        #     canny_edges = torch.from_numpy(canny_edges)
        #     canny_images[i] += canny_edges.to(canny_images[i].device).float()
        
        # print(f"self.backbone: {self.backbone}")
        # features = self.backbone(original_images, gaussian_images, canny_images)
        features = self.backbone(original_images, gaussian_images)
